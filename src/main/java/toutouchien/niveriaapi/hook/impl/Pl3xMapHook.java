package toutouchien.niveriaapi.hook.impl;

import net.pl3x.map.core.Pl3xMap;
import org.bukkit.entity.Player;
import toutouchien.niveriaapi.NiveriaAPI;
import toutouchien.niveriaapi.hook.Hook;

import java.util.UUID;

public class Pl3xMapHook extends Hook {
	private boolean enabled;
	private Pl3xMap plx3Map;

	public Pl3xMapHook(NiveriaAPI plugin) {
		super(plugin);
	}

	@Override
	public void onEnable() {
		this.plx3Map = Pl3xMap.api();
		this.plugin.getSLF4JLogger().info("Hooked into Pl3xMap");
		this.enabled = true;
	}

	@Override
	public void onDisable() {
		this.enabled = false;
		this.plugin.getSLF4JLogger().info("Unhooked from Pl3xMap");
	}

	public void setHidden(UUID uuid, boolean hidden, boolean persistent) {
		if (!this.enabled)
			return;

		this.plx3Map.getPlayerRegistry().optional(uuid)
				.ifPresent(player -> player.setHidden(hidden, persistent));
	}

	public void setHidden(Player player, boolean hidden, boolean persistent) {
		this.setHidden(player.getUniqueId(), hidden, persistent);
	}
}
